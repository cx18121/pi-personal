...
function createSlashSkillAutocompleteProvider(
  ...
  return {
    async getSuggestions(
      ...
      if (skills.length === 0) return currentSuggestions

      const nativeSkillValues = new Set(
        skills.map((skill) => `skill:${skill.name.toLowerCase()}`),
      )
      const currentItems = currentSuggestions?.items.filter(
        (item) => !nativeSkillValues.has(item.value.toLowerCase()),
      )
      const currentWithoutNativeSkills = currentItems?.length
        ? { ...currentSuggestions, items: currentItems }
        : null
      if (options.signal.aborted) return currentWithoutNativeSkills

      const matches = query
        ? filterSkills(skills, query).slice(0, MAX_SUGGESTIONS)
        : skills.slice(0, MAX_SUGGESTIONS)

      if (matches.length === 0) return currentWithoutNativeSkills
      ...
      return mergeAutocompleteItems({
        current: currentWithoutNativeSkills,
        skillItems,
    ...
    applyCompletion(lines, cursorLine, cursorCol, item, prefix) {
      ...
      if (!isSlashSkillCompletion) {
        const textBeforeCursor = currentLine.slice(0, cursorCol)
        const currentPrefix = isPromptStartSlashToken(
          lines,
          cursorLine,
          textBeforeCursor,
          prefix,
