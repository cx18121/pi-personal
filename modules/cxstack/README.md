...
In an active CX session, the model records correction candidates after answering your feedback. It appends a structured marker to the same answer. The correction extension removes the marker before display, stores the interpretation with exact session entry references, and shows an undoable notification. When a user message has broad correction signals but the model omits the marker, the extension records a visible weak candidate for later review. Capture adds no tool call or model turn. `correction_log` exists only for approved Reflect backfills.
