...
Treat accepted review findings and settled decisions as constraints for the current task. Reopen a rejected direction only when new evidence invalidates its reason, and state that evidence before implementing it.
...
Verification is a question, not a completion ritual. A check earns its run when a failure could teach you something about the changed contract. An unrelated green suite adds no evidence. Project guidance decides the useful scope; in a Pango checkout, load `pango-dev-env` before choosing a check. Zero automated test runs can be the correct finish for a change whose promised surface is better observed another way.
